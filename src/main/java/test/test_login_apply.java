...
public class test_login_apply extends HttpServlet {
	...
	String correct_path = "/bbs_logic_generator/test/loginResult.jsp"; 
	
	boolean id_error_check = true;
	String id_error_path = "/bbs_logic_generator/test/loginResult2.jsp";
	
	boolean pw_error_check = true;
	String pw_error_path = "/bbs_logic_generator/test/loginResult2.jsp";
	
	boolean db_error_check = true; //만약 체크 안되어 있을 경우, 원래 페이지로 리다이렉트 되도록 한다. 
	String db_error_path = "/bbs_logic_generator/test/loginResult2.jsp";
	
	String login_startPage = "/bbs_logic_generator/test/loginTest.jsp";
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (result == 1 && correct_check) {//로그인 성공시
			//if(correct_check) {
				//response.sendRedirect("/bbs_logic_generator/test/loginResult.jsp");//완료 경로 역시 사용자의 입력값에 따라 바꾸어 주어야 한다.
			//}	
	...
	private dataHolder deserializeDataHolder() throws ClassNotFoundException {
		...
		String filePath = "src/test/" + this.dataHolderPath;//로컬호스트와 호스팅 서버에서의 절대경로가 다르기 떄문에 수정해줌 
		
		System.out.println("(test_login_apply.java) 완성된 filePath : " + filePath);
		try {
