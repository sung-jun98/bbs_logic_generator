...
//////////////////////////
  ...
  //input/output 태그에 텍스트 대신 input 태그로 사용자 입력을 받을 수 있도록 한다.
			...
			if(connectorInfos.label === "입력값input1"){
				var $form = $('<form>').attr('action', '/bbs_logic_generator/processHTML').attr('method', 'post').attr('enctype', "multipart/form-data").addClass('fileForm');//
			...
			if(connectorInfos.label === "리턴페이지input1"){
				var $form = $('<form>').attr('action', '/bbs_logic_generator/processHTML').attr('method', 'post').attr('enctype', "multipart/form-data").addClass('fileForm');//fileForm : 해당 요소가 있는 오퍼레이터 이름 검색해서 서버로 보냄
				$operator_connector_label = $form.append($('<input>').attr('type', 'file').attr('id', 'loginStartPage').attr('name', 'loginStartPage').css("width", "90px"));
			}
